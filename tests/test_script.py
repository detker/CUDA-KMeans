...
def run_test(filename, data, k, OUT='../data/'):
    ...
    launch_mine('bin', 'gpu1', OUT + filename, 'output_mine_gpu.txt')
    print('✓ KMeans GPU finished')
    
    # Run CPU version
    print("Running CPU version...")
    launch_mine('bin', 'cpu', OUT + filename, 'output_mine_cpu.txt')
    print('✓ KMeans CPU finished')
    
    # Compare results
    clusters_gpu, assignments_gpu = read_output_mine('output_mine_gpu.txt')
    clusters_cpu, assignments_cpu = read_output_mine('output_mine_cpu.txt')
